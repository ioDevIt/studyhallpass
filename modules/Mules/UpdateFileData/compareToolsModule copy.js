...
export  const compareLists=(ASet,BSet,matchKeyFN,compareFields)=>{
    /********** Aset no id field, BSet has id field *********/
        ...
        BSet.forEach((r)=>{if(!(matchKeyFN(r) in bObj)){bObj[matchKeyFN(r)]=[]}; bObj[matchKeyFN(r)].push(r)}) // setting to array so all matches in database are marked as is_active = false
        ...
        for(let i=0;i<bSetLen;i++){
            ...
            if(!(thisBKey in aObj)){
                // if(!(aObj[thisBKey] in both)){
                //     both.push(aObj[thisBKey])
                // }
            // }
            // else{
